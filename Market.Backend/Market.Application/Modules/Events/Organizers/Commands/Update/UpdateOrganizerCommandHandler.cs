...
namespace Market.Application.Modules.Events.Organizers.Commands.Update
{
        ...
        : IRequestHandler<UpdateOrganizerCommand, Unit>
    {
        ...
        {
            if (appCurrentUser.IsUser && appCurrentUser.IsOrganiser != true)
                throw new MarketBusinessRuleException("111", "User can not edit organisers... How did we get here??");

            if ((await ctx.Organizers.Where(x => x.UserId != appCurrentUser.UserId).FirstOrDefaultAsync(ct))?.Id != req.Id && appCurrentUser.IsAdmin == false)
            ...
            var organizerCity = await ctx.Cities
                .Where(x=>x.Id == req.CityId)
                .FirstOrDefaultAsync(ct);

            if (organizerCity is null || organizerCity.IsDeleted == true)
                throw new MarketNotFoundException("The selected city does not exist");

            var userCity = await ctx.Cities
                .Where(x=>x.Id == req.User.CityId)
                .FirstOrDefaultAsync(ct);

            if (userCity is null || userCity.IsDeleted == true)
