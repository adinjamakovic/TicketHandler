    ...
    : IRequestHandler<CreatePersonCommand, int>
{
    public async Task<int> Handle(CreatePersonCommand req, CancellationToken ct)
    {
        ...
        // Login matches the email case-insensitively, so registration has to reject
        // addresses that only differ in casing - otherwise two accounts answer one login.
        var lookupEmail = normalizedEmail.ToLower();

        bool exists = await ctx.Persons
            .AnyAsync(x => x.Email.ToLower() == lookupEmail, ct);
